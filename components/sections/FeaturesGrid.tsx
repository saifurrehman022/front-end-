    ...
    <section className="bg-[#f5f8f5]">
      <Container className="pt-20 pb-10">
        ...
        <h2 className="font-syne font-extrabold text-[clamp(36px,5vw,60px)] text-black mb-0 leading-tight">
          Everything you need.<br /><span className="text-[#7c8a7c]">Nothing you don't.</span>
        </h2>
      </Container>
      <div className="border-t border-[#cfdfcf]">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f, i) => (
            <div key={f.title}
              className="group p-10 border-r border-b border-[#cfdfcf] hover:bg-[#edf3ed] transition-colors duration-200 last:border-r-0 [&:nth-child(4n)]:border-r-0 sm:[&:nth-child(2n)]:border-r-0 lg:[&:nth-child(2n)]:border-r lg:[&:nth-child(4n)]:border-r-0">
              <div className="w-10 h-10 rounded-xl bg-[#e2ebe2] flex items-center justify-center text-black mb-5 group-hover:bg-black group-hover:text-white transition-all duration-200">
                {f.icon}
              </div>
              <h3 className="font-syne font-bold text-[14px] text-black mb-2.5 uppercase tracking-wide">{f.title}</h3>
              <p className="text-[13.5px] text-[#4a564a] font-dm leading-relaxed mb-5">{f.desc}</p>
              <span className="inline-block px-3 py-1 rounded-full bg-[#e2ebe2] text-[11px] font-semibold text-[#4a564a] font-dm group-hover:bg-black group-hover:text-white transition-all duration-200">
                {f.tag}
              </span>
            </div>
          ))}
